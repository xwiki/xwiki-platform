...
/**
 * Contains all the context information for a particular transformation. While an office document is
 * being transformed into a wiki page, this context will act as the storage for that transformation.
 * 
 * @version $Id$
 * @since 1.8M1
 */
public class OfficeImporterContext
{
    ...
    /**
     * Content of the wiki page. This can be html, xhtml, xwiki 2.0 depending on the current phase
     * of transformation.
     */
        ...
        Map<String, String> options, DocumentAccessBridge bridge) throws OfficeImporterException
    {
        ...
        // Determine the document format.
        {
            ...
            if (sourceFormat == null) {
                throw new OfficeImporterException(String.format(
                    "Unable to determine the format of the input file [%s].", sourceFileName));
            }
        }
    ...
    /**
     * This method should be invoked when all the transformations are done. A single
     * {@link OfficeImporterContext} can be finalized only once. After that, calling this method has no
     * effect.
     * 
    ...
    public void finalizeDocument(boolean isPresentation) throws OfficeImporterException
    {
        try {
            if (!finalized) {
                docBridge.setDocumentContent(targetDocument, bufferedContent,
                    "Generated by office importer.", false);
                if (!isPresentation) {
                    for (String artifactName : artifacts.keySet()) {
                        // Filter out the html output.
                        if (!artifactName.equals("output.html")) {
                            addAttachment(artifacts.get(artifactName), artifactName);
                        }
                    }
                }
                finalized = true;
            ...
            throw new OfficeImporterException("Error while finalizing document.", ex);
        }
    }

    /**
     * Adds an attachment to the xwiki document enclosed within this context.
     * 
     * @param sourceData Binary content of the attachment.
     * @param fileName Attachment name.
     */
    public void addAttachment(byte[] sourceData, String fileName) throws OfficeImporterException
    {
        try {
            docBridge.setAttachmentContent(targetDocument, fileName, sourceData);
        } catch (Exception ex) {
            throw new OfficeImporterException(String.format("Error while attaching [%s] to [%s].",
                fileName, targetDocument), ex);
        }
    }

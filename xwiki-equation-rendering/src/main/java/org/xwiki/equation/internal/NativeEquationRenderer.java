...
/**
 * Implementation of the {@link EquationRenderer} component, which uses console commands to build a equation image. Best
 ...
 */
@Component("native")
public class NativeEquationRenderer extends AbstractEquationRenderer implements Initializable
{
    /** Logging helper object. */
    private static final Log LOG = LogFactory.getLog(NativeEquationRenderer.class);
    ...
    {
        this.tempDirectory = new File(this.container.getApplicationContext().getTemporaryDirectory(), "equations");
        ...
        try {
            String texContent =
                "\\documentclass[10pt]{article}\n" + "\\usepackage{amsmath}\n" + "\\usepackage{amsfonts}\n"
                    + "\\usepackage{amssymb}\n" + "\\usepackage{pst-plot}\n" + "\\usepackage{color}\n"
                    + "\\pagestyle{empty}\n" + "\\begin{document}\n" + size.getCommand() + "\n"
                    + wrapEquation(equation, inline) + "\\end{document}\n";
            do {
            ...
            // Write the equation in a tex file

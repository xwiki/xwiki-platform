...
<template>
    <article id="content">
        ...
        <template v-if="pageStatus.withSheet && !serverSideRendering">
          <Template :name="pageStatus.sheet" :document="pageStatus.document" mode="view" />
        </template>
        <template v-else>
...
const pageStatus = ref({ currentContent : "Initial content", css: [], js: [], 
                         html : "", document : null, withSheet : false, sheet: "" })
let logger : Logger;
...
export default {
    setup(props) {
      ...
      if (cristal!=null) {
        ...
        console.error("cristal object not injected properly in content.vue")
      }
      logger?.debug("Sheet is ", pageStatus.value.sheet);
      logger?.debug("With Sheet is ", pageStatus.value.withSheet);
      return { pageStatus : pageStatus, link: "/" + cristal?.getCurrentPage() + "/edit", serverSideRendering : cristal?.getWikiConfig().serverRendering }
    },
    mounted() {
      const cristal = inject<CristalApp>("cristal");

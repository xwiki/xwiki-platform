...
public class RightsManagerPluginApi extends PluginApi
{
    ...
    /**
     * Indicate if provided user has provided rigts level on provided space or page.
     * 
     * @param spaceOrPage the space or page for which to check rights. If null check current wiki
     *            rights.
     * @param level the name of the level right to check ("view", "edit", etc.).
     * @param userName the name of the user.
     * @return true if user has provided rights, false otherwise.
     * @throws XWikiException error when browsing rights.
     */
    public boolean isLevelAllowedForUser(String spaceOrPage, String level, String userName)
        throws XWikiException
    {
        boolean allow = false;
        ...
        try {
            allow =
                RightsManager.getInstance().isLevelAllowed(spaceOrPage, level, userName, true,
                    context);
        } catch (RightsManagerException e) {
            logError("Try to check if user has rights " + QUOTE + level + QUOTE, e);
        }
    ...
    /**
     * Indicate if provided group has provided rigts level on provided space or page.
     * 
     * @param spaceOrPage the space or page for which to check rights. If null check current wiki
     *            rights.
     * @param level the name of the level right to check ("view", "edit", etc.).
     ...
     * @throws XWikiException error when browsing rights.
     */
    public boolean isLevelAllowedForGroup(String spaceOrPage, String level, String groupName)
        throws XWikiException
    {
        boolean allow = false;
        ...
        try {
            allow =
                RightsManager.getInstance().isLevelAllowed(spaceOrPage, level, groupName, false,
                    context);
        } catch (RightsManagerException e) {
            logError("Try to check if group has rights " + QUOTE + level + QUOTE, e);
        }

...
export default class DefaultQueueWorker implements QueueWorker {
    ...
    public constructor(@inject<Logger>("Logger") logger: Logger, @inject<CristalApp>("CristalApp") cristalApp : CristalApp) {
        this.initialize();
        this.logger = logger;
    ...
    public pageLoaded(page : string) {
        try {
          this.logger.debug("Received callback that new document has been loaded", page);
          this.cristalApp.loadPage();
          this.logger.debug("Done callback that new document has been loaded", page);
        } catch (e) {
    ...
    public initialize() {
        try {
            if (this.workerInstance==null) { 
                  // this.workerInstance = new ComlinkWorker<typeof import('./worker')>(new URL('./worker', import.meta.url), {/* normal Worker options*/})
                  let sworker = new Worker();
                  this.workerInstance = Comlink.wrap<MyWorker>(sworker.port);
                  let that = this;
                  this.workerInstance.setPageLoadedCallback(Comlink.proxy(function(page : string) {

...
export class FootnotesService {
  private _footnotes: Reactive<{ translationKey: string; symbol: symbol }[]>;
  constructor() {
   ...
   */
  put(symbol: symbol, translationKey: string) {
    if (
      !this._footnotes.some(
        (footnote) => footnote.translationKey === translationKey,
      )
    ) {

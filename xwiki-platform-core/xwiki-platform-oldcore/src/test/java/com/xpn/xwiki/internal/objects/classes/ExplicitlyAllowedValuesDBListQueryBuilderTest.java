...
public class ExplicitlyAllowedValuesDBListQueryBuilderTest
{
    ...
    public void buildWithDocFullNameAppliesViewableFilter() throws Exception
    {
        String sqlWithFullName =
            "select distinct doc.fullName, doc.web from XWikiDocument doc where doc.name = 'WebHome' order by doc.web";
        ...
        verify(query).setWiki("math");
        // SQL contains "doc.fullName", so the viewable value filter must be applied.

...
var XWiki = (function (XWiki) {
  ...
  Object.extend(XWiki, {
    ...
    resource: {
       ...
       * @param entityType the type of entity specified by the given name; this parameter is optional but very useful
       *   when the passed string is a relative entity reference; e.g. XWiki.EntityType.SPACE for 'xwiki:Main' and
       *   XWiki.EntityType.ATTACHMENT for 'Page@file.txt'
    ...
    displayDocExtra: function (extraID, extraTemplate, scrollToAnchor) {
      ...
      if ($(extraID + "pane").className.indexOf("empty") != -1) {
        ...
        // Determine if JS minification is disabled in the URL. Needed to pass it to the AJAX call to get the right
        // resources on the reply.
        ...
        new Ajax.Request(
          window.docgeturl + '?xpage=xpart&vm=' + extraTemplate + maybeMinifyRequestParameter,
          {
            ...
            onSuccess: function (response) {
              ...
              // from the output and we need to inject the external scripts from the reply (in the DOM's dead), to
              // execute them.
              ...
              container.select('link').each(function (link) {
                var existingElements = head.select(
                  'link[href="' + link.readAttribute('href') + '"][type="' + link.readAttribute('type') + '"]');
                if (existingElements.length == 0) {
                  head.insert(
                    new Element('link', {rel: "stylesheet", type: link.type, href: link.readAttribute('href')}));
                }
              ...
              container.select('script').each(function (script) {
                ...
                if (script.src) {
                  var existingElements = head.select(
                    'script[src="' + script.readAttribute('src') + '"][type="' + script.readAttribute('type') + '"]');
                  if (existingElements.length == 0) {
     ...
     * If a content is passed, add click listener for errors reported in this content (usefull for AJAX requests
     * response) Otherwise make all the document's body errors expandable.
     */
      ...
      $(content || 'body').select(".xwikirenderingerror").each(function (error) {
        ...
        if (description.innerHTML !== "" && description.hasClassName("xwikirenderingerrordescription")) {
          ...
          Event.observe(error, "click", function (event) {
            event.element().closest(".xwikirenderingerror").next(".xwikirenderingerrordescription").toggleClassName(
              "hidden");
          });
        }
      });
            ...
            new Ajax.Request(event.findElement('a').href.replace(/#.*$/, ''), {
              ...
              onSuccess: function (transport) {
                ...
                if (redirect) {
                  ...
                  var notification = new XWiki.widgets.Notification(
                    "$escapetool.javascript($services.localization.render('core.create.popup.loading'))", 'inprogress');
                  // Add the CSS
                  var newStyle = new Element('link', {
              ...
              onFailure: function () {
                new XWiki.widgets.Notification(
                  "$escapetool.javascript($services.localization.render('core.create.ajax.error'))", 'error',
                  {inactive: true}).show();
              }
            });
            event.stop();
 ...
 */
function openHelp() {
  var win = open("http://www.xwiki.org/xwiki/bin/view/Documentation/UserGuide/Features/XWikiSyntax/WebHome?xpage=print",
    "XWikiSyntax", "titlebar=0,width=750,height=480,resizable,scrollbars");
  if (win) {
    win.focus();
  }
}
...
function noaccent(txt) {
  var temp = txt.replace(
    /[\u00c0\u00c1\u00c2\u00c3\u00c4\u00c5\u0100\u0102\u0104\u01cd\u01de\u01e0\u01fa\u0200\u0202\u0226]/g, "A");
  temp =
    temp.replace(/[\u00e0\u00e1\u00e2\u00e3\u00e4\u00e5\u0101\u0103\u0105\u01ce\u01df\u01e1\u01fb\u0201\u0203\u0227]/g,
      "a");
...
function checkAdvancedContent(message) {
  ...
  var myRE2 = new RegExp("(#(set|include|if|end|for)|#(#) Advanced content|public class|/\* Advanced content \*/)",
    "ig");
  ...
  _wrap_shortcut_call: function (callback, shortcut_type) {
    if (shortcut_type === shortcut.type.SEQUENCE) {
      return function (args) {
        ...
        setTimeout(function () {
          shortcut._is_sequence_shortcut_triggered = false;
        }, 500);
...
function BrowserDetect() {
  ...
  this.isNS = ((this.isGecko) ? (ua.indexOf('netscape') != -1) :
    ((ua.indexOf('mozilla') != -1) && !this.isOpera && !this.isSafari && (ua.indexOf('spoofer') == -1) &&
      (ua.indexOf('compatible') == -1) && (ua.indexOf('webtv') == -1) && (ua.indexOf('hotjava') == -1)));
  ...
  this.geckoVersion =
    ((this.isGecko) ? ua.substring((ua.lastIndexOf('gecko/') + 6), (ua.lastIndexOf('gecko/') + 14)) : -1);
  ...
  this.isWin32 = (this.isWin &&
    (ua.indexOf('95') != -1 || ua.indexOf('98') != -1 || ua.indexOf('nt') != -1 || ua.indexOf('win32') != -1 ||
      ua.indexOf('32bit') != -1 || ua.indexOf('xp') != -1));
  this.isMac = (ua.indexOf('mac') != -1);
  this.isUnix =
    (ua.indexOf('unix') != -1 || ua.indexOf('sunos') != -1 || ua.indexOf('bsd') != -1 || ua.indexOf('x11') != -1);
    ...
    if (typeof (XWiki.widgets.Suggest) != "undefined") {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        elements.each(function (element) {
          $(element).select(selector).each(function (item) {
  ...
  document.observe('xwiki:dom:loaded', function () {
    ...
    if (parentInputField) {
      ...
      $('body').observe('click', function (event) {
        if (event.element().descendantOf && !event.element().descendantOf(parentInputSection) && event.element() !=
          parentInputSection && event.element() != editParentTrigger)
        {
          hideParentSection();
        }
      });

...
/**
 * Generate an {@link ExpressionNode}.
 *
 * @version $Id$
 * @since 12.5RC1
 */
@Component(roles = QueryExpressionGenerator.class)
@Singleton
public class QueryExpressionGenerator
{
    ...
    /**
     * For each notification preference of the given user, add a constraint on the events to - have one of the
     * notification types that have been subscribed by the user; - have a date superior to the start date corresponding
     * to this type; - match the custom defined user filters.
     *
    ...
    private AbstractOperatorNode handleEventPreferences(NotificationParameters parameters) throws EventStreamException
    {
        ...
        while (it.hasNext()) {
            ...
            if (!recordableEventDescriptorHelper.hasDescriptor(
                (String) preference.getProperties().get(NotificationPreferenceProperty.EVENT_TYPE), parameters.user)) {
                continue;
            }
            ...
            if (preference.getStartDate().getTime() > 0) {
                preferenceTypeNode = new AndNode(preferenceTypeNode,
                    new GreaterThanNode(value(EventProperty.DATE), value(preference.getStartDate())));
            }

            // Get the notification filters that can be applied to the current preference
            Iterator<NotificationFilter> filterIterator = notificationFilterManager
                .getFiltersRelatedToNotificationPreference(parameters.filters, preference).iterator();
    ...
    private AbstractOperatorNode handleBlackList(NotificationParameters parameters, AbstractOperatorNode topNode)
    {
        if (parameters.blackList != null && !parameters.blackList.isEmpty()) {
            ...
            AbstractOperatorNode node = new NotNode(new InNode(new PropertyValueNode(EventProperty.ID), values));
    ...
    private AbstractOperatorNode handleWiki(NotificationParameters parameters, AbstractOperatorNode topNode)
    {
        ...
        if (parameters.user != null
            ...
            AbstractOperatorNode node = new EqualsNode(new PropertyValueNode(EventProperty.WIKI),
    ...
    private AbstractOperatorNode excludeHiddenEvents(AbstractOperatorNode topNode)
    {
        AbstractOperatorNode node =
            new NotEqualsNode(new PropertyValueNode(EventProperty.HIDDEN), new BooleanValueNode(true));

 ...
 * Macro Service
 */
define('xwiki-wysiwyg-macro-service', [
  'jquery',
  'xwiki-meta'
], function ($, xcontext) {
  'use strict';
  ...
  function getMacros(syntaxId, force) {
    ...
    if (macros && !force) {
      ...
      const macroServiceReference = XWiki.Model.resolve('XWiki.WYSIWYG.MacroService', XWiki.EntityType.DOCUMENT,
        XWiki.currentDocument.documentReference);
      const url = new XWiki.Document(macroServiceReference).getURL('get', $.param({
        outputSyntax: 'plain',
        language: $('html').attr('lang')
      }));
      $.get(url, {

...
class CreatePanelTest extends PageTest
{
    ...
    void createPanel() throws Exception
    {
        ...
        // Verify in particular that the redirection url to has a name that conforms to the naming strategy.
        verify(this.response).encodeURL(endsWith("/xwiki/bin/inline/Panels/" + tool.url(TRANSFORMED_PAGE_TITLE)
    ...
    void createPanelPageAlreadyExist() throws Exception
    {
        ...
        // Verify in particular that the redirection url indicates that the document already exists.
        verify(this.response).encodeURL(endsWith("/xwiki/bin/view/Panels/a?xpage=docalreadyexists"));
    }
}

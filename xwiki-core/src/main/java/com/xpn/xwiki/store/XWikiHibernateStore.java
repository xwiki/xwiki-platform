...
public class XWikiHibernateStore extends XWikiHibernateBaseStore implements XWikiStoreInterface
{
    ...
    protected String createSQLQuery(String queryPrefix, String whereSQL)
    {
        ...
        if (idx >= 0) {
            // With 'WHERE'
            idx = idx + 6;
            result = result.substring(0, idx) + "(doc.hidden <> 1 or doc.hidden is null) and " + result.substring(idx);
        } else {
            // Without 'WHERE'
            ...
            if (idx > 0 && idx < Integer.MAX_VALUE) {
                ...
                result =
                    result.substring(0, idx) + "where doc.hidden <> 1 or doc.hidden is null " + result.substring(idx);
            } else {
                // Without 'WHERE', 'ORDER BY' or 'GROUP BY'... This should not happen at all.
                result = result + " where (doc.hidden <> 1 or doc.hidden is null)";
            }

 ...
 * /!\ This migration is using version 131008000 the reason of the class name is because it has been introduced
 * as a cherry-pick from 14.06 and we use the same name in various artifacts (a listener, different properties,
 * different file names) so we preferred to keep all the names in sync.
 *
 * @version $Id$
 * @since 14.6RC1
 * @since 14.4.3
...
public class R140600000XWIKI19869DataMigration extends AbstractHibernateDataMigration
{
    ...
    {
        // This migration version is correct for branch 13.10.x
        // Note that we kept the class name to be in sync with other parts of the migration (was introduced in 14.06RC1)
        return new XWikiDBVersion(131080000);
    }
        ...
        for (XWikiRCSNodeInfo node : new ArrayList<>(archiveNodes)) {
            try {
                ...
                if (fixPasswordHash(revision, false, false)) {
                    documentArchive.updateArchive(revision, revision.getAuthor(), revision.getDate(),
                        revision.getComment(), revision.getRCSVersion(), context);
                }
            } catch (Exception e) {
                this.logger.warn(

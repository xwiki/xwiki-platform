...
public class DefaultIntegratedLESSCompilerTest
{
    ...
    private LESSResourceReference lessResourceReference = new LESSSkinFileResourceReference("file");
    ...
    {
        // Mocks
        when(cachedIntegratedLESSCompiler.compute(eq(lessResourceReference), eq(false), eq(false), eq(true),
                eq("skin"))).thenReturn("compiled output");
        ...
        // Verify
        verify(cache).set(eq(lessResourceReference), eq(skinReference), eq(colorThemeReference), eq("compiled output"));
    }
    ...
    {
        // Mocks
        when(cachedIntegratedLESSCompiler.compute(eq(lessResourceReference),
                eq(false), eq(false), eq(true), eq("skin"))).thenReturn("compiled output");
        ...
        // Verify
        verify(cache, times(1)).set(eq(lessResourceReference), eq(skinReference), eq(colorThemeReference),
                eq("compiled output"));
    ...
    public void compileSkinFileWhenInCacheButCacheDisabled() throws Exception
    {
        ...
        // Test
        assertEquals("compiled output", mocker.getComponentUnderTest().compile(lessResourceReference, false, false,
                "skin", true));
    ...
    public void compileWhenInCacheAndHTMLExport() throws Exception
    {
        ...
        verify(cachedIntegratedLESSCompiler).compute(eq(lessResourceReference), eq(false), eq(true),
                eq(false), eq("skin"));
    ...
    public void compileWhenError() throws Exception
    {
        ...
        when(cachedIntegratedLESSCompiler.compute(any(LESSResourceReference.class), anyBoolean(), anyBoolean(), 
                anyBoolean(), anyString())).thenThrow(expectedException);

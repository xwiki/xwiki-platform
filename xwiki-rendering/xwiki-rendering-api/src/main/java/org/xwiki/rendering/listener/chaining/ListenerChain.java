...
/**
 * Stores information about the listeners in the chain and the order in which they need to be called. Also
 * sports a feature that allows pushing and popping listeners that are stackable. This feature is useful since
 * listeners can hold stateful information and sometimes you may need to push new versions of them to start
 * with new state information. For example this is used in the XWiki Syntax Renderer when an embedded document event
 * is found to start the rendering for that embedded document using reset state information.
 * 
 * @version $Id$
 * @since 1.8RC1
 */
...
{
    /**
     * The full list of chaining listeners. For each of them we have a stack since the ones that
     * implement the {@link StackableChainingListener} interface can be stacked.
     */
    ...
    /**
     * The ordered list of listeners. We only allow one instance per listener class name so we just need
     * to store the class object and then the instance can be found in {@link #listeners}.
     */
    private List<Class<? extends ChainingListener>> nextListeners = 
        new ArrayList<Class<? extends ChainingListener>>(); 
    
    /**
     * @param listener the chaining listener to add to the chain. If an instance of that listener is already 
     *        present then we stack the new instance instead.
     */
     ...
     * @return the listener instance corresponding to the passed class. Note that the last instance of the stack is returned
     */

...
<template>
    <div>
        ...
        Choose Configuration:
            <ul>
                <li v-for="wikiConfig in configList">
                    <span>{{ wikiConfig.name }} ({{ wikiConfig.baseURL }})</span>
                    <span v-if="wikiConfig.offline"> Offline</span>
                    <span v-if="wikiConfig.designSystem!=''"> Design System: {{ wikiConfig.designSystem }}</span>
                    -
...
export default {
    setup(props) {
        ...
        if (cristal!=null) {
            ...
            configs.forEach((wikiConfig: WikiConfig, key: string) => {
                configList.push(wikiConfig);
            });
            currentConfig = cristal.getWikiConfig().name;
        }
        return { configList : configList, cristal : cristal, currentConfig : currentConfig };
    }
}
</script>
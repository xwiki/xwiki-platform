...
/**
 * Unit tests for {@link org.xwiki.crypto.store.internal.X509CertificateStore}.
 *
 * @version $Id$
 * @since 6.0
 */
...
public class X509CertificateStoreTest
{
    ...
    public MockitoComponentMockingRule<CertificateStore> mocker =
        new MockitoComponentMockingRule<CertificateStore>(X509CertificateStore.class);
    ...
    public void testStoringNewCertificateWithoutKeyIdToDocument() throws Exception
    {
        ...
        verify(certObj, never()).setStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_KEYID, ENCODED_SUBJECTKEYID);
        verify(certObj).setStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_ISSUER, ISSUER);
        verify(certObj).setStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_SERIAL, SERIAL.toString());
        verify(certObj).setStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_SUBJECT, SUBJECT);
        verify(certObj).setLargeStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_CERTIFICATE, ENCODED_CERTIFICATE);
    ...
    public void testUpdatingCertificateWithKeyIdToDocument() throws Exception
    {
        ...
        BaseObject certObj = mock(BaseObject.class);
        when(storeDoc.getXObject(X509CertificateStore.CERTIFICATECLASS, 123)).thenReturn(certObj);
        ...
        verify(certObj, never()).setStringValue(eq(X509CertificateStore.CERTIFICATECLASS_PROP_KEYID), any(String.class));
        verify(certObj, never()).setStringValue(eq(X509CertificateStore.CERTIFICATECLASS_PROP_ISSUER), any(String.class));
        verify(certObj, never()).setStringValue(eq(X509CertificateStore.CERTIFICATECLASS_PROP_SERIAL), any(String.class));
        verify(certObj, never()).setStringValue(eq(X509CertificateStore.CERTIFICATECLASS_PROP_SUBJECT), any(String.class));
        verify(certObj).setLargeStringValue(X509CertificateStore.CERTIFICATECLASS_PROP_CERTIFICATE, ENCODED_CERTIFICATE);

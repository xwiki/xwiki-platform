...
<script lang="ts">
import { Logger, CristalApp, Storage } from '@cristal/api';
import { inject, ref, Ref } from 'vue';
import { Document } from '@cristal/api';
import { ContentTools } from './contentTools';

let logger : Logger | null = null;
let editFieldMap : Map<string, Ref> = new Map<string, Ref>();
let addedHTMLField : Array<string> = [];
// let htmlFieldMap : Map<string, Ref> = new Map<string, Ref>();

export default {
    props : [ "name", "document", "mode", "type"],
    setup(props) {
      ...
      let document : Document =  props.document as Document;
      let value = (document) ? document.get(props.name) : "";
      ...
      if (props.mode=="edit") {
            ...
            storage?.getEditField(document, props.name).then(function (editField : string) {
         ...
         if (props.type=="html") {
            ...
            if (props.name=="html")
               ...
               htmlFieldValue = ""; // cristal?.renderContent(value, "xwiki", "html", cristal?.getWikiConfig());
         }
      }

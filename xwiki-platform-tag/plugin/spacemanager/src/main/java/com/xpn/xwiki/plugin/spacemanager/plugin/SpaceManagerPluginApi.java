...
public class SpaceManagerPluginApi extends PluginApi {
	...
	/**
	 * The plugin will contain SpaceManager api calls to the underlying space
	 * manager plugin. Security will be handled by this plugin.
	 */
	...
	/**
	 * Create a space from scratch It will create an empty space or will copy
	 * the default space template if there is one
	 * 
	 * @param spaceName
	 ...
	 * @return On success returns the newly created space and null on failure
	 */
	public Space createSpaceFromTemplate(String spaceName,
			String templateSpaceName) throws SpaceManagerException {
		Space space;
		try {
			if (!hasProgrammingRights())
				return null;
	...
	/**
	 * Create a space and install an application in the space An application is
	 * handled by the ApplicationManager plugin and can include other
	 * sub-applications
	 * 
	 * @param spaceName
	 ...
	 * @return On success returns the newly created space and null on failure
	 */
	public Space createSpaceFromApplication(String spaceName,
			String applicationName) throws SpaceManagerException {
		Space space;
		try {
			if (!hasProgrammingRights())
				return null;
	 ...
	 * @return On success returns the newly created space and null on failure
	 */
	public Space createSpaceFromRequest(String templateSpace)
			throws SpaceManagerException {
		Space space;
		try {
			if (!hasProgrammingRights())
	...
	public boolean deleteSpace(String spaceName, boolean deleteData)
			throws SpaceManagerException {
		if (!hasProgrammingRights())
			return false;
	...
	/*
	 * private List wrapSpaces(List spacesList) { List list = new ArrayList();
	 * if (spacesList==null) return null; for (int i=0;i<spacesList.size();i++) {
	 * list.add(new Space((Space) spacesList.get(i), context)); } return list; }
	 */
	...
	public List searchSpaces(String fromsql, String wheresql, int nb, int start)
			throws SpaceManagerException {
		List spacesList = getSpaceManager().searchSpaces(fromsql, wheresql, nb,
				start, context);
		return spacesList;
	...
	public List searchSpaceNames(String fromsql, String wheresql, int nb,
		...
		return getSpaceManager().searchSpaceNames(fromsql, wheresql, nb, start,
				context);
	}
	...
	public List searchSpaceNames(String fromsql, String wheresql,
			String ordersql, int nb, int start) throws SpaceManagerException {
	 ...
	 * will get all spaces in which the user is member return space name
	 * 
	 * @param userName
	 * @param role
	 * @return list of space objects
	 */
	public List getSpaces(String userName, String role)
			throws SpaceManagerException {
	...
	/**
	 * Get the list of spaces for a user in a specific role If role is null it
	 * will get all spaces in which the user is member
	 * 
	 * @param userName
	 * @param role
	 * @return list of space names
	 */
	public List getSpaceNames(String userName, String role)
			throws SpaceManagerException {
	 ...
	 * @param space
	 */
	public boolean updateSpaceFromRequest(Space space)
			throws SpaceManagerException {
		if (!hasProgrammingRights())
			return false;
	 ...
	 * 
	 * @param space
	 *            The space in which the search is performed
	 * @param recursive
	 *            Determines if the search is performed in the child spaces too
	 * @param nb
	 *            Number of documents to be retrieved
	 * @return start Pagination option saying at what document index to start
	 *         the search
	 */
	public List getLastModifiedDocuments(String spaceName, boolean recursive,
			int nb, int start) throws SpaceManagerException {
		return getSpaceManager().getLastModifiedDocuments(spaceName, context,
				recursive, nb, start);
	}
	 ...
	 * @param wikiname
	 */
	public void addMember(String spaceName, String wikiname)
			throws SpaceManagerException {
		if (hasProgrammingRights())
	 ...
	 * @throws SpaceManagerException
	 */
	public void removeMember(String spaceName, String wikiName)
			throws SpaceManagerException {
		if (hasProgrammingRights()) {
	 ...
	 * @param wikiname
	 */
	public void addAdmin(String spaceName, String wikiname)
			throws SpaceManagerException {
		if (hasProgrammingRights())
	 ...
	 * @throws SpaceManagerException
	 */
	public void removeAdmin(String spaceName, String wikiName)
			throws SpaceManagerException {
		if (hasProgrammingRights()) {
	...
	public void updateSpaceRights(SpaceApi space, String oldPolicy,
			String newPolicy) throws SpaceManagerException {
		if (hasProgrammingRights()) {
	...
	public void setSubSpaceRights(SpaceApi space, String subSpace)
			throws SpaceManagerException {
		if (hasProgrammingRights()) {

...
-->
  <template>
    <x-alert type="success" :title="title" :description="content">
        <div class="renderedContent" v-html="html">
        </div>
    </x-alert>
  </template>
  <script lang="ts">
  ...
  export default {
      ...
      setup(props) {
        ...
        let htmlFieldValue = cristal?.renderContent(warningMacroData.getMacroContent().toString(), "xwiki", "html", cristal?.getWikiConfig());
        return { title : warningMacroData.getMacroParameter("title"), content : warningMacroData.getMacroContent(), html : htmlFieldValue };
      },
      mounted() {

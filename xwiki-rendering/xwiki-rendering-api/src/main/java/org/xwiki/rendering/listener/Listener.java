...
public interface Listener
{
     ...
     * bold element we would use the {@link #beginFormat(Format)} instead of this event. However for example for an 
     * HTML FORM there's no equivalent event. Note that these events are usually generated by a macro such as the HTML 
     * or XHTML macros.
     * 
    ...
    /**
     * Start a block section that contains an error (for example when a macro fails to execute, is not found,
     * or has some invalid parameters).
     * 

...
public class RSSMacro extends BaseLocaleMacro {
    ...
    public void execute(Writer writer, MacroParameter parameter) throws java.lang.IllegalArgumentException, java.io.IOException {
        ...
        try {
            channel = parseURL(feedURL);
        }
        catch (Exception ex) {
            throw new java.io.IOException("Error processing " + feedURL + ": " + ex.getMessage());
        }
        if (channel == null) throw new IllegalArgumentException("No feed found at " + feedURL);
        if (paramObj.isCss())
        {
            writer.write(NEWLINE);
            writer.write("<div class='rssfeed'>");
        }
        renderImage(channel, writer, paramObj);
        renderTitle(channel, writer, paramObj);
        renderItems(channel, writer, paramObj);
        renderSearch(channel, writer, paramObj);
        writer.write(NEWLINE);
        writer.write(NEWLINE);
    ...
    /**
     * Process the given feedURL and return a <code>Channel</code> object.
     * @param feedURL - the URL to read
     * @return - a <code>org.apache.commons.digester.rss.Channel</code> object.
     * @throws Exception - if anything goes wrong during the processing.
     */
    protected Channel parseURL(URL feedURL) throws Exception {
        Channel channel = null;
        DeluxeRSSDigester digester = new DeluxeRSSDigester();
        try {
            channel = (Channel) digester.parse(feedURL.openStream());
        } catch (NoClassDefFoundError e)
        {
            throw new IllegalStateException("Some libraries not installed: " + e.getMessage());
        }
     ...
     */
    private void renderItems(Channel channel, Writer writer, RSSMacroParameters paramObj) throws java.io.IOException
    {
        Item[] items = channel.getItems();
        int max = paramObj.evalCount(items.length);
        for (int i = 0; i < max; i++) {
            renderItem(items[i], writer, paramObj);
        }
    }
     ...
     * @throws java.io.IOException from calls to <code>writer.write()</code>
     */
    private void renderItem(Item item, Writer writer, RSSMacroParameters paramObj) throws java.io.IOException
    {
        StringBuffer buf = new StringBuffer();
        if (paramObj.isCss()){
            renderItemCSS(item, buf, paramObj);
        } else {
            renderItemDefault(item, buf, paramObj);
        }
     ...
     * @param paramObj our parameter helper object
     */
    private void renderItemDefault(Item item, StringBuffer buf, RSSMacroParameters paramObj) {
        buf.append(NEWLINE).append("* ");
        if (item.getLink() != null) { buf.append("{link:"); }
        ...
        if (paramObj.isFull() && item.getDescription() != null)
        {
            buf.append(NEWLINE).append("{quote}")
               .append(NEWLINE).append(item.getDescription())
     ...
     */
    private void renderItemCSS(Item item, StringBuffer buf, RSSMacroParameters paramObj) {
        ...
        if (item.getLink() != null)
        {
            buf.append("<a class='rssitemtitle' href=\"")
               .append(Encoder.escape(item.getLink()))
               .append("\">");
        }
        buf.append(item.getTitle());
        if (item.getLink() != null) { buf.append("</a>"); }
     ...
     * @throws java.io.IOException from calls to <code>writer.write()</code>
     */
    private void renderTitle(Channel channel, Writer writer, RSSMacroParameters paramObj) throws java.io.IOException
    {
        StringBuffer buf = new StringBuffer();
        if (paramObj.isCss())
        {
            renderTitleCSS(channel, buf);
        } else {
            renderTitleDefault(channel, buf);
        }
        writer.write(buf.toString());
    }
     ...
     * @param buf the StringBuffer we're using to prepare the output
     */
    private void renderTitleDefault(Channel channel, StringBuffer buf) {
        buf.append("__");
        if (channel.getLink() != null) buf.append("{link:");
        buf.append(channel.getTitle());
        if (channel.getLink() != null)
        {
            buf.append("|")
               .append(channel.getLink())
               .append("}");
        }
        buf.append("__\n\n");
    }
     ...
     * TODO Figure out how to stop Radeox from filtering the URLs
     */
    private void renderTitleCSS(Channel channel, StringBuffer buf) {
        buf.append(NEWLINE).append("<div class='rsschanneltitle'>");
        if (channel.getLink() != null)
        {
            buf.append("<a class='rsschanneltitle' href=\"")
                    .append(Encoder.escape(channel.getLink()))
                    .append("\">");
        }
        buf.append(channel.getTitle());
        if (channel.getLink() != null) buf.append("</a>");
     ...
     * @throws java.io.IOException from calls to <code>writer.write()</code>
     */
    private void renderImage(Channel channel, Writer writer, RSSMacroParameters paramObj) throws java.io.IOException
    {
        if (channel.getImage() == null) return;
        if (!(paramObj.isImg())) return;

        Image rssImage = channel.getImage();
        StringBuffer buf = new StringBuffer(NEWLINE + "{image:");
        buf.append("img=").append(rssImage.getURL());
        buf.append("|link=").append(channel.getLink());
     ...
     * @throws java.io.IOException from calls to <code>writer.write()</code>
     */
    private void renderSearch(Channel channel, Writer writer, RSSMacroParameters paramObj) throws java.io.IOException
    {

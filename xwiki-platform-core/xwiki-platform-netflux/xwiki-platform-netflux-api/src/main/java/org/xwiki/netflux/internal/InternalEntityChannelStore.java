...
public class InternalEntityChannelStore
{
    ...
    public synchronized EntityChannel createChannel(EntityReference entityReference, List<String> path)
    {
        ...
        // Generate a new identifier for the channel
        String channelKey = this.idGenerator.generateChannelId();

        // Create the new entity channel
        EntityChannel channel = createChannel(channelKey, entityReference, path);

        // Notify about the new channel
        this.observation.notify(new EntityChannelCreatedEvent(channelKey, entityReference, path), null);
     ...
     * @return information about the created channel
     */
    public synchronized EntityChannel createChannel(String channelKey, EntityReference entityReference,
        List<String> path)
    {
        // Create a new netflux channel
        this.channelStore.create(channelKey);

        EntityChannel channel = new EntityChannel(entityReference, path, channelKey);

        this.entityChannels.addChannel(channel);
    ...
    {
        return this.entityChannels.getChannels().values().stream().flatMap(List::stream)

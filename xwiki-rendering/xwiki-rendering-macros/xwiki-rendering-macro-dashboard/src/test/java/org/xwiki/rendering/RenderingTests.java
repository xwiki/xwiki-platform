...
public class RenderingTests extends TestCase
{
    ...
    private static void setUpSkinExtensionStubs(ComponentManagerTestSetup testSetup) throws Exception
    {
        ...
        {
            {
                String cssPath = "uicomponents/dashboard/dashboard.css";
                allowing(ssfxMock).use(with(cssPath));
                allowing(ssfxMock).use(with(cssPath), with(any(Map.class)));
                String cssPathColumns = "uicomponents/container/columns.css";
                allowing(ssfxMock).use(with(cssPathColumns));
                allowing(ssfxMock).use(with(cssPathColumns), with(any(Map.class)));
            }
        });

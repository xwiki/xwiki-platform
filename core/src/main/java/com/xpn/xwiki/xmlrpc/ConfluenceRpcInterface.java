 ...
 *
 * <p>Note: Lots of the Javadoc comments below are borrowed from the
 * <a href="http://www.atlassian.com/software/jira/docs/api/rpc-jira-plugin/latest/index.html?com/atlassian/jira/rpc/xmlrpc/XmlRpcService.html">
 * Confluence Javadoc</a>.</p>
 *
 * @version $Id: $
...
public interface ConfluenceRpcInterface
{
     ...
     * on the version of the server.
     * 
     * @param token the authentication token retrieved when calling the login method
     * @return the Server information such as Server base URL and Server version.
     *         The returned map contains the fields from {@linkServerInfo}.
     ...
     * Create a new space.
     * @param token the authentication token retrieved when calling the login method
     * @param spaceProperties Map containing all informations, we need to create
     *          a new space. We need the following keys:
     *              - key "name": the name of the space
     *              - key "key": the space key
     *              - key "description": the space description
     ...
     * 
     * For adding, the Page given as an argument should have
     *      - space
     *      - title
     *      - content
     *      
     * For updating, the Page given should have 
     *      - id
     *      - space
     *      - title
     *      - content
     ...
     *  
     * If 'content' is provided, then that is rendered as if it 
     * were the body of the page (useful for a 'preview page' function). 
     * If it's not provided, then the existing content of the 
     * page is used instead (ie useful for 'view page' function).
     * 
     * @param token the authentication token retrieved when calling the login method
     * @param spaceKey in which space is our page
     * @param pageId id of page to get rendered HTML
     * @param content if this is set, it will replace the original content
     *      for rendering

...
public availableConfigurations : Map<string, WikiConfig>;
    ...
    constructor(@inject("ExtensionManager") extensionManager : ExtensionManager,
        @inject("SkinManager") skinManager: SkinManager,
        @multiInject("VueTemplateProvider") vueTemplateProviders: VueTemplateProvider[],
    ...
    setCurrentPage(newPage : string) {
        if (this.page.name!=newPage) {
            ...
            history.pushState({ page : newPage }, newPage, "/" + this.wikiConfig.name + "/#/" + newPage + "/view");
    ...
    switchConfig(configName : string) : void {
        ...
        if (wikiConfig) {
            ...
            if (!this.isElectron) {
                ...
                window.location.href = "/" + configName + "/#/" + wikiConfig.homePage + "/";
            } else {
    ...
    async loadPage() {
        try {
            ...
            if (this.getWikiConfig().isSupported("jsonld")) {
                const pageContentData = await this.getWikiConfig().storage.getPageContent(this.page.name, "jsonld");
                this.page.document = pageContentData.document;
                this.page.source = pageContentData.document.get("text");
                if (pageContentData.html=="") {
                    this.page.html = await this.renderContent(this.page.source, pageContentData.syntax, "html", this.getWikiConfig());
                ...
                const pageContentData = await this.getWikiConfig().storage.getPageContent(this.page.name, "html");
    ...
    async run() {
        ...
        let routes = [
            { path: '/', component: this.skinManager.getTemplate("content") } as RouteRecordRaw,
            { path: '/:page/view', component: this.skinManager.getTemplate("content") } as RouteRecordRaw,
            { path: '/:page/edit', component: this.skinManager.getTemplate("edit")} as RouteRecordRaw,
            { path: '/xwiki/search', component: this.skinManager.getTemplate("search")} as RouteRecordRaw,
        ...
        if (perfMode) {
            routes = [
                ...
                { path: '/X', component: this.skinManager.getTemplate("perfX") } as RouteRecordRaw,
                { path: '/empty', component: this.skinManager.getTemplate("perfempty") } as RouteRecordRaw,
                { path: '/vuetify', component: this.skinManager.getTemplate("perfvuetify") } as RouteRecordRaw,
                { path: '/dsfr', component: this.skinManager.getTemplate("perfdsfr") } as RouteRecordRaw,
                { path: '/sl', component: this.skinManager.getTemplate("perfsl") } as RouteRecordRaw,
              ]
        }
        ...
        const vueComponents = this.container.getAll<VueTemplateProvider>("VueTemplateProvider")
        for (const vueComponentId in vueComponents) {
            const vueComponent = vueComponents[vueComponentId];
            this.logger?.debug("Found vue component ", vueComponent.getVueName(), vueComponent);
            ...
            this.logger?.debug("Ready to register components of ", vueComponent.getVueName())
            vueComponent.registerComponents(this.app);
        }

        const uixComponents = this.container.getAll<VueTemplateProvider>("UIXTemplateProvider")
        for (const uixComponentId in uixComponents) {
            const uixComponent = uixComponents[uixComponentId];
            this.logger?.debug("Found vue component ", uixComponent.getVueName(), uixComponent);
            // registering additional components

            this.logger?.debug("Ready to register components of ", uixComponent.getVueName())
        ...
        history.replaceState({ page : this.getCurrentPage() }, "", "/" + this.wikiConfig.name + "/#/" + this.getCurrentPage() + "/view");
    ...
    getUIXTemplates(extensionPoint : string) : Array<Component> {
        ...
        try {
            this.logger?.debug("Searching for UIX with extension Point", extensionPoint);
            const uixComponents = this.getContainer().getAllNamed<UIXTemplateProvider>("UIXTemplateProvider", extensionPoint);
            this.logger?.debug("All uix components for extension point ", extensionPoint, uixComponents);

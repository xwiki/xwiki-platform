...
-->
<template>
  <div class="livedata-entry-selector-info-bar" v-if="selectedCount > 0">
    <span v-if="!logic.entrySelection.isGlobal">
      {{ $t("livedata.selection.infoBar.selectedCount", [selectedCount]) }}
    </span>
    <span
      v-else-if="
        logic.entrySelection.isGlobal && !logic.entrySelection.deselected.length
      "
    >
    ...
    <span v-else>
      {{
        $t("livedata.selection.infoBar.allSelectedBut", [
          logic.entrySelection.deselected.length,
        ])
      }}
    </span>
  </div>
  ...
  computed: {
    ...
    selectedCount() {
      if (this.logic.entrySelection.isGlobal) {
        return (
          this.data.data.count - this.logic.entrySelection.deselected.length
        );
      } else {

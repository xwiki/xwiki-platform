 ...
 * "full" the isdiff column in the database should be false). Note: This migrator should only be executed if the
 * R4359XWIKI1459 one has already been executed (i.e. if the database is in version < 4360). This is because this
 * current migrator is because of a bug in R4359XWIKI1459 which has now been fixed.
 * 
 * @version $Id$
 */
...
{
    /** logger. */
    private static final Logger LOGGER = LoggerFactory.getLogger(R6079XWIKI1878Migrator.class);
    ...
    public void migrate(XWikiHibernateMigrationManager manager, final XWikiContext context) throws XWikiException
    {
        ...
        manager.getStore(context).executeWrite(context, true, new XWikiHibernateBaseStore.HibernateCallback<Object>()
        {
            ...
            public Object doInHibernate(Session session) throws HibernateException, XWikiException
            {
                try {
                    Query query =
                        session.createQuery("select rcs.id, rcs.patch, doc.fullName "
                    ...
                    ((XWikiHibernateVersioningStore) context.getWiki().getVersioningStore()).setSession(session,
                        context);
                    ((XWikiHibernateVersioningStore) context.getWiki().getVersioningStore()).setTransaction(

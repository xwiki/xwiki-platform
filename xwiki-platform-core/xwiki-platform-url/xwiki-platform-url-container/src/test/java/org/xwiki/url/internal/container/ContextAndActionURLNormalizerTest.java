...
public class ContextAndActionURLNormalizerTest
{
    ...
    public void normalizeFromVirtualWikiRequestPreservesWikiPath() throws Exception
    {
        ...
        when(req.getPathInfo()).thenReturn("/dev/Space/Page");
    ...
    @Test
    public void normalizeUsesConfiguredDefaultServletMapping() throws Exception
    {
        when(this.xwikiCfg.getProperty("xwiki.webapppath")).thenReturn("xwiki");
        when(this.urlConfiguration.getEntityPathPrefix()).thenReturn("do");

        assertEquals("/xwiki/do/one/two", this.mocker.getComponentUnderTest().normalize(this.testURL).serialize());
    }

...
export class CristalAppLoader extends CristalLoader {
    ...
    public loadApp(config : any, isElectron : boolean) {
        ...
        const wikiConfig = this.cristal.getAvailableConfigurations().get(configName);
        if (wikiConfig==null) {
            if (!isElectron)
    ...
    public async launchApp(forceStaticMode : boolean, configPath : string, isElectron : boolean) {
        ...
        this.container.bind<CristalApp>("CristalApp").to(DefaultCristalApp).inSingletonScope();
        this.cristal = this.container.get<CristalApp>("CristalApp") as DefaultCristalApp;

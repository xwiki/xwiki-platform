...
import org.xwiki.search.solr.internal.RemoteSolr;
...
{
    /**
     * The network alias to use for the Solr container.
     */
    public static final String NETWORK_ALIAS = "xwikisolr";

    /**
     * The network port to use for the Solr container.
     */
    public static final int NETWORK_PORT = 8983;
    ...
    private void startSolrContainer(TestConfiguration testConfiguration, MavenResolver mavenResolver) throws Exception
    {
        ...
        if (StringUtils.isNotBlank(solrTag)) {
            ...
            // No tag specified, use "9"
            // TODO: add support for Solr 10
            solrContainer = new SolrContainer("solr:9");
        }
        ...
        // Get project's Solr major version
        String solrMajorVersion = mavenResolver.getPropertyFromCurrentPOM("solr.major.version");

        // Create the cores
        for (String coreName : findAllCores()) {
            addCoreToContainer(coreName, solrCoresPath, coreArtifacts, solrMajorVersion);
        }

        // Configure the container network
        solrContainer.withNetwork(Network.SHARED).withNetworkAliases(NETWORK_ALIAS);

        // We don't need Zookeeper for our use case since we only manipulate a single Solr instance
        solrContainer.withZookeeper(false);

        // Copy the cores file and make sure Solr has full control over them
        solrContainer.withCopyFileToContainer(MountableFile.forHostPath(solrCoresPath, 0777), "/var/solr/data");

        // XWiki needs the analysis-extras module
        solrContainer.addEnv("SOLR_MODULES", "analysis-extras");
        ...
        if (testConfiguration.getServletEngine().isOutsideDocker()) {
            baseURL = getBaseURL(solrContainer.getHost(), solrContainer.getSolrPort());
        } else {
            baseURL = getBaseURL(NETWORK_ALIAS, NETWORK_PORT);
        }
        testConfiguration.getSolrMode().setBaseURL(baseURL);
    }

    private String getBaseURL(String host, int port)
    {
        return String.format("http://%s:%s/solr", host, port);
    }

    private void addCoreToContainer(String coreName, String solrCoresPath, Map<String, File> coreArtifacts,
        String solrMajorVersion) throws Exception
    {
        String coreDirectoryPath =
            solrCoresPath + RemoteSolr.DEFAULT_CORE_PREFIX + '_' + coreName + '_' + solrMajorVersion;
        File coreDirectory = new File(coreDirectoryPath);
        if (!coreDirectory.exists()) {
            coreDirectory.mkdirs();
        }

        File coreConfigurationFile = coreArtifacts.get(coreName);
        try (InputStream coreContent = new FileInputStream(coreConfigurationFile)) {
            LOGGER.info("    Unzipping configuration [{}] to [{}]", coreConfigurationFile.getAbsolutePath(),
                coreDirectoryPath);

            copyCoreConfiguration(coreContent, coreDirectory.toPath());
        }
    }
    ...
    {
        return mavenResolver.getArtifactResolver().resolveArtifact(new DefaultArtifact("org.xwiki.platform",
            getCoreArtifactId(coreName), "zip", mavenResolver.getPlatformVersion())).getArtifact().getFile();
    }
    ...
    private Set<String> findAllCores()
    {
        ...
        LOGGER.info("    Found the following Solr core initializers: {}", namedValues);

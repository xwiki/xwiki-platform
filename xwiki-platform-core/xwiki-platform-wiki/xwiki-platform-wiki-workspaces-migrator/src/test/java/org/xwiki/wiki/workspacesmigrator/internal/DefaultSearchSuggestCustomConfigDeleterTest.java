...
@ComponentTest
class DefaultSearchSuggestCustomConfigDeleterTest
{
    @InjectMockComponents
    private DefaultSearchSuggestCustomConfigDeleter configDeleter;
    ...
    @Test
    void deleteSearchSuggestCustomConfig() throws Exception
    {
        XWikiDocument searchSuggestConfigDoc = mock(XWikiDocument.class);
        when(this.xwiki.getDocument(eq(new DocumentReference("mainWiki", "XWiki", "SearchSuggestConfig")),
        ...
        when(objConfig3.getStringValue("query")).thenReturn(
        ...
        when(objConfig4.getStringValue("query")).thenReturn(
            "XWiki.XWikiServerClass.wikiprettyname:__INPUT__* AND object:WorkspaceManager.WorkspaceClass");
        ...
        when(objConfig6.getStringValue("query")).thenReturn(
            "XWiki.XWikiServerClass.wikiprettyname:__INPUT__* AND object:WorkspaceManager.WorkspaceClass");
        when(objConfig6.getStringValue("url")).thenReturn("bad URL");

        // Run
        this.configDeleter.deleteSearchSuggestCustomConfig("mainWiki");
        ...
        verify(this.xwiki).saveDocument(searchSuggestConfigDoc,
            "Remove object previously introduced by WorkspaceManager.Install", this.xcontext);
    }
}

...
public class XWikiContextCacheKeyFactoryTest
{
    ...
    public void getCacheKey() throws Exception
    {
        ...
        when(skinDirectoryGetter.getFileSystemSkin(anyString())).thenReturn("fsskin");
        when(urlFactory.createSkinURL(anyString(), eq("fsskin"), any(XWikiContext.class))).thenReturn(
                new URL("http://url"));

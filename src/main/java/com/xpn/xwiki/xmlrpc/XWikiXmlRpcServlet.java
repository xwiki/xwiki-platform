...
public class XWikiXmlRpcServlet extends XmlRpcServlet {
	...
	protected PropertyHandlerMapping newPropertyHandlerMapping(URL url)
		...
		mapping.setTypeConverterFactory(getXmlRpcServletServer()
		...
		RequestProcessorFactoryFactory factory
			...
			protected Object getRequestProcessor(Class pClass,
				...
				if (proc instanceof RequestInitializableHandler) {
					RequestInitializableHandler initProc
						= (RequestInitializableHandler) proc;
					try {
						ServletRequest request = ((RequestInitializableHandler.Config)
								pRequest.getConfig()).getRequest();

...
<template>
    <template v-if="pageStatus.withSheet">
        <Template :name="pageStatus.sheet" :document="pageStatus.document" mode="edit" />
    </template>
    <template v-else>
        <iframe id="editui" :src="link" 
...
export default {
    setup(props) {
        ...
        if (cristal) {
            ...
            let link = cristal?.getWikiConfig().baseURL +  "/bin/view/" + cristal?.getCurrentPage().replaceAll(".", "/") + "?xpage=plain&htmlHeaderAndFooter=true&minify=false&outputTitle=true#edit";
    ...
    mounted() {
        ...
        window.addEventListener("message", (event) => { 
            ...
            if (event.data && event.data.type && (event.data.type=="cancel" || event.data.type=="save")) {

...
/**
 * Override the default implementation of hte WikiModel XHTML parser for handling HTML headers.
 * We need to do this in order to handle the <code>id</code> attribute that we generate in our
 * XHTML rendered.
 * 
 * @version $Id$
 * @since 1.6RC1
...
public class XWikiHeaderTagHandler extends HeaderTagHandler
{
    ...
    protected void begin(TagContext context)
	{
        ...
        // TODO: If there's an ID attribute then check if it corresponds to the default section ID generated by the 
        // XWiki ID generator. If so remove it so that it's not displayed. We need to do this because the
        // XWiki HTML renderer automatically generates an ID for sections.

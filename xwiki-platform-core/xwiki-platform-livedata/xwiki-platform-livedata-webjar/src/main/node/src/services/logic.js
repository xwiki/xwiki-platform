...
export class Logic {
  constructor(element, $) {
    ...
    try {
      ...
      this.contentTrusted = scriptEl.getAttribute('data-config-content-trusted') === 'true';
    } catch (e) {
    ...
    this.setEditBus(editBusInit(this));
    ...
    // Registers panels once the translations have been loadded as they are otherwise hard to
    // update.
    this.translationsLoaded().finally(() => {
      this.registerPanel({
        id: "propertiesPanel",
        title: i18n.global.t("livedata.panel.properties.title"),
        name: i18n.global.t("livedata.dropdownMenu.panels.properties"),
        icon: "list-bullets",
      ...
      this.registerPanel({
        id: "sortPanel",
        title: i18n.global.t("livedata.panel.sort.title"),
        name: i18n.global.t("livedata.dropdownMenu.panels.sort"),
        icon: "table_sort",
      ...
      this.registerPanel({
        id: "filterPanel",
        title: i18n.global.t("livedata.panel.filter.title"),
        name: i18n.global.t("livedata.dropdownMenu.panels.filter"),
        icon: "filter",
  ...
  */
  ...
  getDisplayerDescriptor(propertyId) {
    ...
    const customDisplayerDescriptor = jsonMerge({}, typeDescriptor?.displayer,
  ...
  getFilterDescriptor(propertyId) {
    ...
    const customFilterDescriptor = jsonMerge({}, typeDescriptor?.filter,
  ...
  isPropertyEditable(propertyId) {
    ...
    return propertyDescriptor &&
      (propertyDescriptor.editable !== undefined ? propertyDescriptor.editable :
  ...
  getLastIndexOfPage(pageIndex) {
    ...
    if (0 <= pageIndex && pageIndex < this.getPageCount()) {
      return Math.min(this.getFirstIndexOfPage(pageIndex) + this.data.query.limit,
  ...
  reorderProperty(from, toIndex) {
    ...
    this.data.query.properties.splice(toIndex, 0,
  ...
  isPropertySortable(propertyId) {
    ...
    const propertyTypeDescriptor = this.getPropertyTypeDescriptor(propertyId);
    return propertyDescriptor &&
      (propertyDescriptor.sortable !== undefined ? propertyDescriptor.sortable :
  ...
  isPropertyFilterable(propertyId) {
    ...
    const propertyTypeDescriptor = this.getPropertyTypeDescriptor(propertyId);
    return propertyDescriptor &&
      (propertyDescriptor.filterable !== undefined ? propertyDescriptor.filterable :

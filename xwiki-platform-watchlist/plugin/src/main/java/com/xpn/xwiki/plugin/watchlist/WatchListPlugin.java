...
public class WatchListPlugin extends XWikiDefaultPlugin implements XWikiPluginInterface {
    ...
    protected void initEmailTemplate(XWikiContext context) throws XWikiException {
        ...
        if (needsUpdate) {
            ...
            mail.setStringValue("subject", "XWiki : Watchlist Updates, #set($format=\"dd/MM/yyyy\")" +
                    "#if ($interval == 1)#set($format=\"${format} hh:mm\")#end$xwiki.formatDate($xwiki.date, $format)");
            mail.setLargeStringValue("text", "Hello $pseudo,\n" +
                    "This message is sent by XWiki. Here are the documents in your watchlist that have been created " +
            ...
            mail.setLargeStringValue("html", "<b>Hello $pseudo,</b><br/>\n" +
                    "<i>This message is sent by XWiki. Here are the documents in your watchlist that have been " +
                    "created or updated since the last notification :</i><br/>\n" +
                    "<br/>\n" +
                    "<table width=\"100%\">\n" +
    ...
    /**
     * Sets a largeString property in the localUser's WatchList Object, then saves the localUser's profile
     *
     * @param localUser XWiki User
    ...
    /**
     * Add the specified element (document or space) to the corresponding list in the localUser's WatchList
     *
    ...
    public boolean addWatchedElement(String localUser, String newWatchedElement,
        ...
        List watchedItems = isSpace ?
                this.getWatchedSpaces(localUser, context) : this.getWatchedDocuments(localUser, context);
    ...
    /**
     * Remove the specified element (document or space) from the corresponding list in the localUser's WatchList
     *
     * @param localUser XWiki User
     ...
     * @throws XWikiException If the WatchList Object cannot be retreived or if the localUser's profile cannot be saved
     */
    ...
    public List getWatchListWhatsNew(String localUser, XWikiContext context) throws XWikiException {
        BaseObject watchListObject = this.getWatchListObject(localUser, context);
        String watchedDocuments = watchListObject.getLargeStringValue("documents").trim().replaceFirst("^,", "")
                .replaceAll(",", "','");
        String watchedSpaces = watchListObject.getLargeStringValue("spaces").trim().replaceFirst("^,", "")
                .replaceAll(",", "','");

 ...
 */
@ComponentTest
class WikiUserFromXEMMigrationTest
{
    @InjectMockComponents(role = HibernateDataMigration.class)
    private WikiUserFromXEMMigration wikiUserFromXEMMigration;
    ...
    void upgradeWorkspace() throws Exception
    {
        ...
        when(this.xwiki.getDocument(eq(memberGroupRef), any(XWikiContext.class))).thenReturn(memberGroupDoc);
        ...
        verify(this.xwiki).saveDocument(oldDescriptorDocument, "[UPGRADE] Remove the old WorkspaceManager.WorkspaceClass"
            + " object.", this.xcontext);
        ...
        verify(this.xwiki).saveDocument(memberGroupDoc,
            "Upgrade candidacies from the old Workspace Application to the new Wiki Application.", this.xcontext);
    }

    @Test
    void upgradeOldWorkspaceTemplate() throws Exception
    {
        // Mocks about the descriptor
        ...
        when(this.xwiki.exists(
            eq(new DocumentReference("workspacetemplate", "XWiki", "ManageWorkspace")),

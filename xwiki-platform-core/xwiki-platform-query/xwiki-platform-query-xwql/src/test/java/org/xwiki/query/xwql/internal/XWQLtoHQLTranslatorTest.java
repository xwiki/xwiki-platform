...
class XWQLtoHQLTranslatorTest
{
    @Mock
    private DocumentAccessBridge dab;

    private final XWQLtoHQLTranslator translator = new XWQLtoHQLTranslator()
    {
        @Override
    ...
    {
        when(this.dab.getPropertyType(any(), any())).thenReturn("StringProperty");
        when(this.dab.getPropertyType(any(), eq("number"))).thenReturn(null);
    ...
    {
        String output = this.translator.translate(input);
    ...
    void property() throws Exception
    {
        assertTranslate(
            ...
            where ( user_email1.value = 'some' ) and doc.fullName=user.name \
            and user.className='XWiki.XWikiUsers' \
            and user_email1.id.id=user.id and user_email1.id.name='email'""");
    }
    ...
    void shortForm() throws Exception
    {
        ...
        assertTranslate("from doc.object('XWiki.XWikiUs\u00E9rs') as user where user.email = 'some'",
            ...
            where ( user_email1.value = 'some' ) and doc.fullName=user.name \
            and user.className='XWiki.XWikiUs\u00E9rs' and user_email1.id.id=user.id \
            and user_email1.id.name='email'""");
    }

    @Test
    void objDeclInWhere() throws Exception
    {
        assertTranslate("where doc.object('XWiki.XWikiUs\u00E9rs').email = 'some'",
            ...
            where ( _o1_email2.value = 'some' ) and doc.fullName=_o1.name \
            and _o1.className='XWiki.XWikiUs\u00E9rs' and _o1_email2.id.id=_o1.id \
            and _o1_email2.id.name='email'""");
    }

    @Test
    void objDeclInWhereWithTwoInstances() throws Exception
    {
        assertTranslate(
    ...
    void orderBy() throws Exception
    {
        ...
        assertTranslate("from doc.object(XWiki.XWikiUsers) as user order by user.firstname",
            """
            select doc.fullName from XWikiDocument as doc , BaseObject as user , \
            StringProperty as user_firstname1 \
            where 1=1 and doc.fullName=user.name and user.className='XWiki.XWikiUsers' \
            and user_firstname1.id.id=user.id and user_firstname1.id.name='firstname' \
            order by user_firstname1.value""");
    ...
    @Test
    void groupByAndOrderBy() throws Exception
    {
        assertTranslate(
            """
            select obj.property1, count(obj.property2) from Document doc, \
            ...
            from XWikiDocument as doc , BaseObject as obj , StringProperty as obj_property11, \
            StringProperty as obj_property22 \
        ...
        assertTranslate(
            """
            select obj.property1, sum(obj.property2) from Document doc, \
        ...
        assertTranslate(
            """
            select obj.property1, avg(obj.property2) from Document doc, \
        ...
        assertTranslate(
            """
            select obj.property1, max(obj.property2) from Document doc, \
        ...
        assertTranslate(
            """
            select obj.property1, min(obj.property2) from Document doc, \
    ...
    @Test
    void internalProperty() throws Exception
    {
    ...
    void lists() throws Exception
    {
        ...
        assertTranslate("from doc.object('XWiki.ArticleClass') as a where :cat member of a.category",
            """
            select doc.fullName from XWikiDocument as doc , BaseObject as a , \
            DBStringListProperty as a_category1 \
            where ( :cat in elements( a_category1.list ) ) \
            and doc.fullName=a.name and a.className='XWiki.ArticleClass' \
            and a_category1.id.id=a.id \
            and a_category1.id.name='category'""");
        // StringListProperty
        assertTranslate("from doc.object('XWiki.Class') as c where c.stringlist like '%some%'",
            """
            select doc.fullName from XWikiDocument as doc , BaseObject as c , \
            StringListProperty as c_stringlist1 \
            where ( c_stringlist1.textValue like '%some%' ) \
            and doc.fullName=c.name \
            and c.className='XWiki.Class' \
            and c_stringlist1.id.id=c.id \
            and c_stringlist1.id.name='stringlist'""");
        // return DBStringListProperty
        assertTranslate(
            "select distinct a.category from Document as doc, doc.object('XWiki.ArticleClass') as a",
            """
            ...
            where 1=1 and doc.fullName=a.name and a.className='XWiki.ArticleClass' \
            and a_category1.id.id=a.id \
            and a_category1.id.name='category'""");

        assertTranslate(
            """
            select a.category from Document as doc, doc.object('XWiki.ArticleClass') as a \
    ...
    @Test
    void customMapping() throws Exception
    {
        // one CM prop
        assertTranslate(
            "select doc from Document as doc, doc.object('Custom.Mapping') as c where c.cmprop='some'",
            """
        ...
        assertTranslate(
            ...
            """
            select doc from XWikiDocument as doc , BaseObject as c , Custom.Mapping as cCM1, \
            StringProperty as c_prop2 \
            where ( cCM1.cmprop = 'some' and c_prop2.value = 1 ) \
            and doc.fullName=c.name and c.id=cCM1.id \
            and c_prop2.id.id=c.id \
            and c_prop2.id.name='prop'""");
    }

    @Test
    void methodsInLike() throws Exception
    {
        assertTranslate(
            """
            ...
            StringProperty as obj_first_name1 \

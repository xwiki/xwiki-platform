...
import { DefaultLogger, DefaultLoggerConfig, LoggerConfig } from '@cristal/api';
...
export class CristalLoader {
    ...
    public async loadExtensionManager(staticExtensionManager : boolean) : Promise<void> {
        ...
        try {
            if (staticExtensionManager) {
                ...
                this.logger?.debug("Dynamic extension Manager. Ready to import ", importPath);
            ...
            this.logger?.error("Exception while loading extension Manager from ", importPath, e);
        }
    }

    public async loadExtensions(staticExtensionManager : boolean, staticBuild: boolean) : Promise<ExtensionManager | null> {
        this.logger?.debug("Start app");
        await this.loadExtensionManager(staticExtensionManager);
        
        const extensionManager = this.container.get<ExtensionManager>("ExtensionManager");
        this.logger?.debug("Extension Manager is ", extensionManager);

        if (extensionManager!=null) {
            if (!staticBuild) {
                extensionManager.setRemoteExtensionsConfig("/apps/", "main.bundle.dev.js")
                for (let extName in this.extensionList) {
    ...
    public initializeContainer() {
        ...
        this.container.bind<LoggerConfig>("LoggerConfig").to(DefaultLoggerConfig).inSingletonScope();

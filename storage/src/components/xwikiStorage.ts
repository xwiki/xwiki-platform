...
import { Logger, PageData, DefaultPageData, Storage, WikiConfig } from '@cristal/api';
...
export class XWikiStorage implements Storage {
    ...
    getPageRestURL(page: string, syntax : string): string {
        ...
        return this.wikiConfig.baseURL  + this.wikiConfig.baseRestURL + "&page=" + page + "&format=" + syntax;
    ...
    getImageURL(page : string, image : string) : string {
        if (page=="")
          page = "Main.WebHome"
        const imageURL = this.wikiConfig.baseURL + "/bin/download/" + page.replace(".", "/") + "/" + image;
    ...
    async getPanelContent(panel: string, contextPage: string, syntax: string): Promise<PageData> {
        const url = this.wikiConfig.baseURL  + "/rest/wiki30/panel?media=json" + "&page=" + contextPage + "&panel=" + panel;
        this.logger?.debug("XWiki Loading url", url );
        const response = await fetch(url, {cache: "no-store"});
        const json = await response.json();
    ...
    async getEditField(document : Document, fieldName : string): Promise<string> {     
        ...
        try {
          let fieldMapping : any = document.get("xwikiMapping")[fieldName];
          ...
          const url = this.wikiConfig.baseURL  + "/bin/get/" + document.getIdentifier().replaceAll(".", "/")
                    + "?xpage=display&mode=edit&type=" + type + "&property=" + document.get("className") + "." + xwikiFieldName;
          this.logger?.debug("XWiki Loading Field url", url );
          const response = await fetch(url, {cache: "no-store"});
          return await response.text();
        } catch (e) {
          this.logger?.error("Exception looking for edit field for field", fieldName, e)
          return "";
        }
    }    

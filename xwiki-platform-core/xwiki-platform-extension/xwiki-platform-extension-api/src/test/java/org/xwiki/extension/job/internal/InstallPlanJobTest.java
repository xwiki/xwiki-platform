...
public class InstallPlanJobTest extends AbstractExtensionHandlerTest
{
    ...
    {
        ExtensionPlan<InstallRequest> plan = installPlan(TestResources.REMOTE_SIMPLE_ID, null);

        Assert.assertEquals(1, plan.getExtensionTree().size());

        ExtensionPlanNode node = plan.getExtensionTree().iterator().next();

        Assert.assertEquals(TestResources.REMOTE_SIMPLE_ID, node.getAction().getExtension().getId());
    ...
    {
        ExtensionPlan<InstallRequest> plan = installPlan(TestResources.REMOTE_WITHRDEPENDENCY_ID, null);
    ...
    public void testInstallPlanWithCoreDependencyOnRoot() throws Throwable
    {
        ...
        Assert.assertEquals(Action.NONE, node.getAction().getAction());
        Assert.assertTrue(childnode.getChildren().isEmpty());
    }
    ...
    {
        ExtensionPlan<InstallRequest> plan = installPlan(TestResources.REMOTE_WITHLDEPENDENCY_ID, null);

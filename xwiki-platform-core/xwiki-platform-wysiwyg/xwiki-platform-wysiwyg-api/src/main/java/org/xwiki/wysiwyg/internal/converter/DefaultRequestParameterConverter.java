...
public class DefaultRequestParameterConverter implements RequestParameterConverter
{
    ...
    @Override
    public Optional<javax.servlet.ServletRequest> convert(javax.servlet.ServletRequest request,
        javax.servlet.ServletResponse response) throws IOException
    {
        Optional<ServletRequest> jakarta = convert(ServletBridge.toJakarta(request), ServletBridge.toJakarta(response));

        Optional<javax.servlet.ServletRequest> javax;
        if (jakarta.isPresent()) {
            javax = Optional.of(ServletBridge.toJavax(jakarta.get()));
        } else {
            javax = Optional.empty();
        }
    ...
    @Override
    public RequestParameterConversionResult convert(javax.servlet.ServletRequest request)
    {
        return new RequestParameterConversionResult(convert(ServletBridge.toJakarta(request)));
    }

...
Object.extend(XWiki, {
  ...
  insertSectionEditLinks: function() { 
      ...
      if (XWiki.docsyntax == "xwiki/2.0" && XWiki.contextaction == "view") {
          ...
          var sectioncount = 1;
          // We can't use element.select() since it does not allow to retrieve comment nodes.
          var nodes = $("xwikicontent").childNodes;
          // We don't have to use a macro depth variable since multiple macro levels will generate only one comment.
          var withinmacro = false;
          
          for (var i = 0; i < nodes.length; i++) {
              var node = $(nodes[i]);
              
              // Deactivate "edit section" links insertion when we are in content generated by a macro.
              ...
              // Insert "edit section" links after level 1 and level 2 headings.
              if (withinmacro == 0 && (node.nodeName == "H1" || node.nodeName == "H2")) {                  

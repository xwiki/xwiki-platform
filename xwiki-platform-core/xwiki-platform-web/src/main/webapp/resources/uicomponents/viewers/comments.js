 ...
 */
viewers.Comments = Class.create((function () {

  // Private fields and methods
  const commentNbrRegex = /.+_(\d+)/;
   ...
   */
  const extractCommentNbr = function (comment) {
   ...
   */
  const hasCommentNbr = function (comment) {
      ...
      $$(this.xcommentSelector).each(function (item) {
        ...
        item.observe('click', function (event) {
          ...
          if (item.disabled) {
            ...
            comment.hide();
            const commentNbr = extractCommentNbr(comment);
            this.reloadEditor({

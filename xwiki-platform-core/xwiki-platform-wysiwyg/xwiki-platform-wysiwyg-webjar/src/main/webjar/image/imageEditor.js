...
define('xwiki-wysiwyg-image-editor-translation-keys', {
  prefix: 'imageEditor.modal.',
  keys: [
    'changeImage.button',
    'loadFail.message',
    'title',
...
define('xwiki-wysiwyg-image-style-client', ['jquery'], function($) {
  'use strict';
  let cachedResultDefault;
  let cachedResult;
  ...
  function getImageStyle(styleId) {
    return loadImageStyles().then(function (results) {
      const filteredResults = results.imageStyles.filter(function (imageStyle) {
        return imageStyle.type === styleId;
      });
      if (filteredResults.length === 0) {
        throw new Error(`Unable to find an image style with id [${styleId}]`);
      } else {
...
define('xwiki-wysiwyg-image-editor', [
  'jquery',
  'xwiki-wysiwyg-modal',
  'xwiki-wysiwyg-image-style-client',
  'xwiki-l10n!xwiki-wysiwyg-image-editor-translation-keys',
  'xwiki-selectize'
  ...
  // Used to store the default style configuration once loaded by loadImageStylesDefault.
    ...
    function addChangeImageButton(insertButton, modal) {
      const selectImageButton = $('<button type="button" class="btn btn-default pull-left"></button>')
        .text(translations.get('changeImage.button'))
    ...
    function checkDimensions(modal) {
      loadImage(modal).then(function (image) {
        ...
        if (widthOutscaled || heightOutscaled) {
          ...
          errorField.text(translations.get('outscaleWarning', width + 'x' + height + 'px'));
        } else {
    ...
    function initialize(modal) {
      ...
      if (params.newImage) {
        modal.find('.modal-footer .btn-primary').text(translations.get('insertButton'));
      } else {
        modal.find('.modal-footer .btn-primary').text(translations.get('updateButton'));
      }
      
      if (modal.data('initialized')) {
        updateForm(modal);
      } else {
        const url = new XWiki.Document(XWiki.Model.resolve('XWiki.WYSIWYG.ImageEditorService',
    ...
    return $modal.createModalStep({
      ...
      acceptLabel: translations.get('insertButton'),

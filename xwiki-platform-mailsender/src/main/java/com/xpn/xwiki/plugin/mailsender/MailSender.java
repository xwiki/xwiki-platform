...
public interface MailSender
{
     ...
     * A helper method for Velocity scripts since we cannot create Java objects from Velocity.
     *
     * @param xwiki the XWiki object used to get the default values from the XWiki Preferences
     *        ("smtp_server" and "smtp_from"). 
    ...
    /**
     * Generic method for sending emails. The passed Mail object has to be populated by the caller
     * to set the correct fields. All the other <code>sendHtmlXXX()</code> and
     * <code>sendTextXXX()</code> methods are specialized helper versions of this generic method.
     *
     ...
     *         context under the "error" key.
     */
    int sendMail(Mail mail);

    /**
     * Generic method for sending emails. The passed Mail object has to be populated by the caller
     * to set the correct fields. The passed Mail Configuration allows the user to override the
     * default connection properties (SMTP host, SMTP port, SMTP from, etc).
     * All the other <code>sendHtmlXXX()</code> and <code>sendTextXXX()</code> methods are

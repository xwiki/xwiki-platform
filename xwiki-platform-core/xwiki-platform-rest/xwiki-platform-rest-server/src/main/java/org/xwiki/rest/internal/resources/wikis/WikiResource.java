 ...
 * @version $Id$
 */
@Component("org.xwiki.rest.internal.resources.wikis.WikiResource")
@Path("/wikis/{wikiName}")
public class WikiResource extends XWikiResource
{
     ...
     * @throws XWikiException if there was an error during import.
     */
    @POST
    public Wiki importXAR(@PathParam("wikiName") String wikiName,
            @QueryParam("backup") @DefaultValue("false") Boolean backup,
            @QueryParam("history") @DefaultValue("add") String history, InputStream is) throws XWikiException
    {
        if (!wikiExists(wikiName)) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }

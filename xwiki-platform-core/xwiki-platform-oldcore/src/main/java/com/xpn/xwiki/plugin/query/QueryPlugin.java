...
public class QueryPlugin extends XWikiDefaultPlugin implements IQueryFactory
{
    ...
    /**
     * @deprecated This version si buggy since it use the initial context of the plugin and not the current context. For
     *             the store itself this is not major as it is a singleton
     */
    @Deprecated
    ...
    public IQuery xpath(String q, IQueryFactory qf) throws XWikiException
    {
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("create xpath query: " + q);
        if (isHibernate(qf))
            try {
    ...
    {
        if (LOGGER.isDebugEnabled())
            LOGGER.debug("create JCRSQL query: " + q);
        if (isHibernate(qf))
            try {
                return new HibernateQuery(parse(q, Query.SQL), qf);
            } catch (InvalidQueryException e) {
                throw new XWikiException(XWikiException.MODULE_XWIKI_PLUGINS, XWikiException.ERROR_XWIKI_UNKNOWN,
                    "Invalid jcrsql query: " + q);
            }
        return null;
    }
     ...
     * @param order - properties for sort, separated by ','; order: ascending/descending after prop. name, or +/-
     *            before. if null - not sort
     * @throws XWikiException
     * @deprecated This version si buggy since it use the initial context of the plugin and not the current context
     */
    @Deprecated
    ...
    public IQuery getAttachment(String docname, String attachname, String order, IQueryFactory qf)
        throws XWikiException
    {
    ...
    public IQuery getObjects(String docname, String oclass, String prop, String order, IQueryFactory qf)
        throws XWikiException
    {
        return xpath("/" + getXPathName(docname) + "/obj/" + getXPathName(oclass) + getPropertyXPath(prop)
            + getOrderXPath(order), qf);
    }

    protected String getXWikiName(String name)
    {
        int is = name.indexOf('['), ip = name.indexOf('/');
        if (is < 0 || ip < is)
    ...
    public String makeQuery(XWikiQuery query) throws XWikiException
    {
        ...
        if (classes.size() > 1)
            throw new XWikiException(XWikiException.MODULE_XWIKI_STORE,
                XWikiException.ERROR_XWIKI_STORE_SEARCH_NOTIMPL, "Search with more than one class is not implemented");
        if (classes.size() == 0)
            return "";
    ...
    public List search(XWikiQuery query, IQueryFactory qf) throws XWikiException
    {
        ...
        return doclist;
        /*
         * if (doclist==null) return null; List list = new ArrayList(); for (int i=0;i<doclist.size();i++) {
         * list.add(((XWikiDocument)doclist.get(i)).getFullName()); } return list;
         */
    }
}

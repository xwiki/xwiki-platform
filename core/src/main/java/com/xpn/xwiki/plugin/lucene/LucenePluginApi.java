...
public class LucenePluginApi extends Api
{
     ...
     * languages. <p> With virtual wikis enabled in your xwiki installation this will deliver
     * results from all virtuall wikis. For searching in a subset of your virtual wikis see {@link
     * #getSearchResults(String,String,String,com.xpn.xwiki.api.XWiki)} </p>
     *
     * @param query query entered by the user
     * @param languages comma separated list of language codes to search in, may be null to search
     * all languages. Language codes can be: <ul> <li><code>default</code> for content having no
    ...
    /**
     * Searches the configured Indexes using the specified lucene query for documents in the given
     * languages belonging to one of the given virtual wikis. <p> Using this method only makes sense
     * with virtual wikis enabled. Otherwise use {@link #getSearchResults(String,String,
     *com.xpn.xwiki.api.XWiki)} instead. </p>
     *
     * @param query query entered by the user
